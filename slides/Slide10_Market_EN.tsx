  ...
  const phases: Phase[] = [
    {
      ...
      chinese: "Physical Anchor",
      focus: "Penetration",
      color: "green",
      icon: "monitor",
      desc: "Solve 'Alt-Tab Hell'. Every device is a data collector.",
      hidden: "Record human-computer interaction paths through hybrid landing.",
      business: "Hardware Sales",
      metric: "DAA",
      metricLabel: "Daily Active Actions"
    },
    {
      id: 2,
      num: "02",
      title: "Workflow Network",
      chinese: "Workflow Network",
      focus: "Accumulation",
      color: "teal",
      icon: "database",
      desc: "100K users = 100K operation patterns. Launch macro store.",
      hidden: "Build proprietary behavior graph—validated enterprise data.",
      business: "SaaS + 30% Commission",
      metric: "Graph",
      metricLabel: "Proprietary Data"
    },
    {
      id: 3,
      num: "03",
      title: "Agent Standard",
      chinese: "Agent Standard",
      focus: "Monopoly",
      color: "blue",
      icon: "radio",
      desc: "OpenAI's choice: $$ high-risk vision or ¢ Violoop API.",
      hidden: "Become the default I/O gateway for all AI agents.",
      business: "API Tollbooth",
      metric: "Protocol",
      metricLabel: "Industry Standard"
    }
  ];
                  ...
                  >
                    ...
                    {isActive && (
                      ...
                      >
                        <div className={`text-[8px] md:text-[9px] ${colors.text} uppercase mb-1 flex items-center gap-1`}>
                          <Icon name="eye" size={8} /> Hidden Goal
                        </div>
                    ...
                    <div className={`mt-2 md:mt-3 pt-1.5 md:pt-2 border-t ${isActive ? 'border-zinc-700' : 'border-zinc-800'}`}>
                      <div className="flex justify-between items-center">
                        <span className="text-[8px] md:text-[9px] text-zinc-500">Business Model</span>
      ...
      <div className="pt-3 md:pt-4 pb-16 md:pb-8 px-4 md:px-8 z-10">
        <div className="flex flex-col md:flex-row items-center justify-center gap-2 md:gap-4 p-3 md:p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl max-w-3xl mx-auto">
          ...
          <p className="text-zinc-400 text-xs md:text-sm text-center md:text-left">
            "Tesla sells cars to build an energy network. <span className="text-white font-bold">Violoop sells hardware to build a behavior graph.</span>"
          </p>
          <div className="flex items-center gap-2 md:gap-3 md:pl-4 md:border-l border-zinc-700">
            <span className="text-green-400 font-bold text-sm md:text-base">$10B</span>
            <Icon name="arrowRight" size={14} className="text-zinc-600" />
            <span className="text-blue-400 font-bold text-sm md:text-base">$100B</span>
          </div>
        </div>
      </div>
    </div>
