...
const commands = [
  { cmd: '$ git status', result: '修改: src/app.ts' },
  { cmd: '$ docker ps', result: '容器: api_prod 运行中' },
  { cmd: '$ npm test', result: '✓ 42 个测试通过' },
];
  ...
  const layers: Layer[] = [
    {
      ...
      color: "blue",
      icon: "terminal",
      description: "对于支持 CLI/API 的应用（Git、Docker、数据库），我们完全绕过 UI。",
      examples: ["终端命令", "REST APIs", "数据库查询", "Git 操作"],
      visual: "cli"
    },
    {
      id: 1,
      ...
      color: "green",
      icon: "layers",
      description: "macOS AX API / Windows UIA / 浏览器 DOM。我们将 UI 读取为结构化数据。",
      examples: ["原生应用", "网页浏览器", "系统对话框", "表单输入"],
      visual: "tree"
    },
    {
      id: 2,
      name: "第三层：视觉回退",
      subtitle: "Computer Use（最后手段）",
      coverage: "10%",
      accuracy: "~85%",
      latency: "2-5秒",
      color: "yellow",
      icon: "eye",
      description: "仅用于边缘情况：游戏、画布应用、自定义渲染 UI。",
      examples: ["游戏界面", "画布编辑器", "老旧软件", "自定义控件"],
      visual: "vision"
    }
  ];
      ...
      <div className="py-3 md:py-4 border-t border-zinc-900 mt-3 md:mt-4 z-10">
        <div className="flex flex-wrap items-center justify-center gap-4 md:gap-8 px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-400">无障碍 60%</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded bg-yellow-500" />
            <span className="text-xs text-zinc-400">视觉 10%</span>
          </div>
          <div className="text-sm text-white font-bold md:ml-4">= 100% 覆盖</div>
        </div>
      </div>
    </div>
  );
