...
const Slide09_BusinessModel: React.FC = () => {
  ...
  const steps: Step[] = [
    { id: 0, title: "Demonstrate & Learn", desc: "Show once, automate forever", icon: "eye", color: "blue", progress: "33%" },
    { id: 1, title: "Graph Synthesis", desc: "Convert to deterministic logic", icon: "gitBranch", color: "green", progress: "66%" },
    { id: 2, title: "Publish & Share", desc: "Dynamic workflows for everyone", icon: "share", color: "purple", progress: "100%" }
  ];
    ...
    <div className="w-full h-full flex flex-col relative overflow-y-auto md:overflow-hidden px-4 md:px-8">
          ...
          <div className="flex-1 flex flex-col justify-center max-w-md">
            ...
            <div className="mt-4 md:mt-6 p-3 md:p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="text-white font-bold text-sm">Semantic Wrapper</div>
                  <div className="text-zinc-500 text-xs">Every workflow becomes deterministic logic. No fragile pixel matching.</div>
                </div>
              </div>
            </div>
          </div>
                        ...
                        <div className="w-full h-full relative">
                          <div className="absolute top-2 left-2 right-2 flex items-center justify-between px-2 md:px-3 py-1.5 md:py-2 bg-zinc-900 border border-zinc-800 rounded-lg">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-red-400 text-[10px] md:text-xs font-mono">Recording</span>
                            </div>
                            <span className="text-zinc-500 text-[10px] md:text-xs">Learning: "Export Invoice"</span>
                          </div>
                          <div className="absolute inset-4 md:inset-8 top-10 md:top-14 bg-zinc-800 rounded-lg overflow-hidden">
                            <div className="h-5 md:h-6 bg-zinc-700 flex items-center px-2 gap-1.5">
                              ...
                              <span className="text-[8px] md:text-[9px] text-zinc-500 ml-2">Invoice Manager</span>
                            </div>
                            ...
                            <motion.div className="absolute bottom-2 md:bottom-3 right-2 md:right-3 px-2 md:px-3 py-1 md:py-1.5 bg-blue-500 rounded text-white text-[8px] md:text-[10px] font-bold" animate={{ scale: [1, 0.95, 1] }} transition={{ delay: 2, duration: 0.2, repeat: Infinity, repeatDelay: 2.8 }}>Export PDF</motion.div>
                          ...
                          <motion.div className="absolute bottom-2 right-2 text-blue-400 text-[10px] md:text-xs" animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 1, repeat: Infinity }}>Recording ⚡</motion.div>
                        </div>
                      )}
                      {activeStep === 1 && (
                        <div className="w-full h-full relative flex">
                          ...
                          <div className="w-32 md:w-44 bg-black/80 border-l border-zinc-800 p-2 md:p-3 font-mono text-[7px] md:text-[8px] overflow-hidden">
                            ...
                            <div className="space-y-0.5 text-green-400">
                              ...
                              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="pl-1"><span className="text-purple-400">"name"</span>: <span className="text-amber-400">"Export Invoice"</span>,</motion.div>
                              ...
                              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="pl-2 text-zinc-500">{`{ "click": "Export" }`}</motion.div>
                              ...
                              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="pl-1"><span className="text-purple-400">"deterministic"</span>: <span className="text-blue-400">true</span></motion.div>
                            ...
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="mt-2 pt-2 border-t border-zinc-800 text-green-400 flex items-center gap-1">
                              <Icon name="check" size={8} /> <span>Compiled</span>
                            </motion.div>
                          </div>
                          <motion.div className="absolute bottom-2 left-2 text-green-400 text-[10px] md:text-xs" animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 1, repeat: Infinity }}>Synthesizing ⚡</motion.div>
                        </div>
                      )}
                      {activeStep === 2 && (
                        <div className="w-full h-full p-2 flex flex-col">
                          ...
                          <div className="flex items-center justify-between mb-2 pb-2 border-b border-zinc-800">
                            <div className="flex items-center gap-2"><Icon name="layers" size={12} className="text-purple-400" /><span className="text-white text-[10px] md:text-xs font-bold">Workflow Hub</span></div>
                            <div className="flex items-center gap-2 md:gap-3">
                              <div className="text-center"><div className="text-white text-[10px] md:text-xs font-bold">12.4k</div><div className="text-zinc-600 text-[6px] md:text-[7px]">Workflows</div></div>
                              <div className="text-center"><div className="text-white text-[10px] md:text-xs font-bold">3.2k</div><div className="text-zinc-600 text-[6px] md:text-[7px]">Creators</div></div>
                            </div>
                          </div>
                          {/* Workflow list */}
                          <div className="flex-1 space-y-1 md:space-y-1.5 overflow-hidden">
                            {[
                              { name: "Export Invoice to PDF", author: "Sarah K.", uses: "2.3k", apps: ["QB", "Chrome"] },
                              { name: "Figma → Slack Updates", author: "Mike D.", uses: "1.8k", apps: ["Figma", "Slack"] },
                              { name: "Daily Standup Notes", author: "Acme Team", uses: "956", apps: ["Zoom", "Notion"] },
                              { name: "GitHub PR Review Flow", author: "DevOps Inc", uses: "2.1k", apps: ["GitHub", "Linear"] },
                            ].map((w, i) => (
                              <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="flex items-center gap-2 p-1 md:p-1.5 bg-zinc-800/50 border border-zinc-700 rounded-lg hover:border-purple-500/30 transition-colors">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="text-zinc-500 text-[6px] md:text-[7px]">{w.author} • {w.uses} uses</div>
                                </div>
                          ...
                          <div className="mt-2 pt-2 border-t border-zinc-800 flex items-center justify-between">
                            <div className="flex items-center gap-2 md:gap-3 text-[7px] md:text-[8px]">
                              ...
                              <span className="text-green-400">+47 new</span>
                              <span className="text-blue-400">89k runs</span>
                            </div>
      ...
      <div className="flex flex-wrap justify-center gap-4 md:gap-8 py-3 md:py-4 border-t border-zinc-900 z-10 px-4">
        <div className="flex items-center gap-4 md:gap-6">
          {[
            { label: 'Record', color: 'bg-blue-500' },
            { label: 'Synthesize', color: 'bg-green-500' },
            { label: 'Share', color: 'bg-purple-500' }
          ].map((item, i) => (
        ...
        <p className="text-zinc-500 text-xs">Every workflow becomes a <span className="text-white font-bold">reusable app</span></p>
      </div>
    </div>
  );
};
