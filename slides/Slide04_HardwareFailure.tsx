    ...
    <div className="w-full h-full flex flex-col relative overflow-hidden px-4 md:px-8">
          ...
          >
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-white font-bold text-sm">桌面端</span>
              </div>
              <div className="flex gap-2 mb-2 flex-wrap">
                <span className="px-2 py-0.5 bg-green-900/30 text-green-400 text-[9px] font-bold uppercase rounded">算力 ✓</span>
                <span className="px-2 py-0.5 bg-green-900/30 text-green-400 text-[9px] font-bold uppercase rounded">控制 ✓</span>
              </div>
              <div className={`text-[11px] leading-relaxed mb-2 ${activeDevice === 'desktop' ? 'text-red-300' : 'text-zinc-500'}`}>
                智能体依附于操作系统。<br />
                <span className="text-white font-semibold">电脑休眠 → 智能体死亡</span>
              </div>
              <div className="flex items-center gap-2 pt-2 border-t border-zinc-800">
                <Icon name="xCircle" size={14} className="text-red-500" />
                <span className="text-red-400 text-[10px] font-mono font-bold">无状态保持</span>
              </div>
            </motion.div>

            {/* Label below */}
            <span className="mt-3 text-[10px] font-mono text-zinc-600 uppercase tracking-wider">无外部观察者</span>
          </motion.div>

          {/* VIOLOOP - Center (The Solution) - Chinese */}
          <motion.div
          ...
          >
            ...
            >
              ...
              <div className="flex flex-wrap gap-1.5 mb-2">
                <span className={`px-2 py-0.5 text-[9px] font-bold uppercase rounded ${showVioloop ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-zinc-800 text-zinc-600'}`}>编排</span>
                <span className={`px-2 py-0.5 text-[9px] font-bold uppercase rounded ${showVioloop ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-zinc-800 text-zinc-600'}`}>持久</span>
                <span className={`px-2 py-0.5 text-[9px] font-bold uppercase rounded ${showVioloop ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-zinc-800 text-zinc-600'}`}>外部</span>
                <span className={`px-2 py-0.5 text-[9px] font-bold uppercase rounded ${showVioloop ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-zinc-800 text-zinc-600'}`}>语音控制</span>
              </div>
              <div className={`text-[11px] leading-relaxed mb-2 ${showVioloop ? 'text-green-100' : 'text-zinc-600'}`}>
                移动端的独立性 + 桌面端的控制深度 = <span className="font-bold text-white">控制平面</span>
              </div>
              <div className="flex items-center gap-2 pt-2 border-t border-zinc-800">
                <Icon name="checkCircle" size={14} className={showVioloop ? 'text-green-500' : 'text-zinc-700'} />
                <span className={`text-[10px] font-mono font-bold ${showVioloop ? 'text-green-400' : 'text-zinc-700'}`}>外部观察者</span>
              </div>
            </motion.div>
          </motion.div>

          {/* MOBILE - Right - Chinese */}
          <motion.div
            className="relative flex flex-col items-center"
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-white font-bold text-sm">移动端</span>
              </div>
              <div className="flex gap-2 mb-2 flex-wrap">
                <span className="px-2 py-0.5 bg-green-900/30 text-green-400 text-[9px] font-bold uppercase rounded">便携 ✓</span>
                <span className="px-2 py-0.5 bg-green-900/30 text-green-400 text-[9px] font-bold uppercase rounded">常开 ✓</span>
              </div>
              <div className={`text-[11px] leading-relaxed mb-2 ${activeDevice === 'mobile' ? 'text-red-300' : 'text-zinc-500'}`}>
                只能消费，不能创作。<br />
                <span className="text-white font-semibold">无法编排复杂任务。</span>
              </div>
              <div className="flex items-center gap-2 pt-2 border-t border-zinc-800">
                <Icon name="xCircle" size={14} className="text-red-500" />
                <span className="text-red-400 text-[10px] font-mono font-bold">I/O带宽不足</span>
              </div>
            </motion.div>

            {/* Label below */}
            <span className="mt-3 text-[10px] font-mono text-zinc-600 uppercase tracking-wider">仅消费</span>
          </motion.div>
        </div>
      </div>
      ...
      >
        <div className="inline-flex flex-wrap items-center justify-center gap-2 md:gap-4 px-4 md:px-6 py-2 md:py-3 bg-zinc-900/80 border border-zinc-800 rounded-full">
          <span className="text-zinc-500 text-xs md:text-sm">缺失的拼图：</span>
          <span className="text-green-400 font-bold text-xs md:text-sm">外部观察者</span>
          <span className="text-zinc-600">+</span>
          <span className="text-green-400 font-bold text-xs md:text-sm">编排中枢</span>
          <span className="text-zinc-600">+</span>
          <span className="text-green-400 font-bold text-xs md:text-sm">上帝模式</span>
        </div>
      </motion.div>
    </div>
  );
